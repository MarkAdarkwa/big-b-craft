...
-- Run this once in the Supabase SQL Editor for project wqupyceppmlsmwhajyvc
-- (or against whichever Supabase project your .env points at).
...
UPDATE public.products SET
  image = 'product-' || id::text,
  gallery = ARRAY['product-' || id::text, 'product-' || id::text],
  colors = jsonb_build_array(jsonb_build_object('name', c.name, 'swatch', c.swatch, 'image', 'product-' || id::text))
FROM (VALUES
